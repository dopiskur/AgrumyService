...
namespace Agrumy.Api.Dal.Interface
{
    ...
    public interface IDiscoveryRepository
    {
        ...
        Task<IList<DiscoveryResult>> DiscoveryResultsGetAsync(int? tenantId, int? unitId, int? zoneId, int? parcelId = null);

        /// Same best-pick as above, narrowed to one DiscoveredApMac for the Register flow - tenantId scopes eligible scanning devices, so a caller can't resolve a winner from another organization's report.

...
public class DeviceFirmwareOtaTests
{
    ...
    private DeviceConfig RegisterAndGetConfig(Device device, IList<DeviceCommand>? pendingOutboxItems = null)
    {
        ...
        // BuildDeviceConfigAsync always reads ServerConfig and the device's own organization (UtcOffsetSeconds); no ScheduleTimeZone configured, so the response's offset is 0/UTC.
    ...
    /// Reset rides along on this same Register/Config response, but - unlike FirmwareUpdate, which waits for the device to confirm the new version before clearing - it clears the instant it's included, since a device that resets() wipes itself before it could ever confirm anything.
    [Fact]

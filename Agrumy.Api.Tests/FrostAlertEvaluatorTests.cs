...
public class FrostAlertEvaluatorTests
{
    ...
    private FrostAlertEvaluator NewEvaluator(string? apiKey = "test-key") =>
        new(_serverConfig.Object, _tenants.Object, _units.Object, _farmParcels.Object, _simulation.Object, _devices.Object, _users.Object, _weatherClient.Object, _dispatcher.Object,
    ...
    private void SetupState(WeatherLocationState state) =>
        _tenants.Setup(t => t.WeatherLocationStateGetAsync(TenantId, Lat, Lon)).ReturnsAsync(state);
    ...
    private static WeatherLocationState BaseState(bool frostPredicted = false, DateTimeOffset? checkedAtUtc = null) => new()
    {
        TenantID = TenantId,
        Latitude = Lat,
        Longitude = Lon,
    ...
    public async Task No_ApiKey_Configured_Does_Nothing()
    {
        ...
        // Strict mocks: every dependency call past ServerConfigGetAsync would throw if reached.
    }
    ...
    public async Task Location_Not_Fully_Set_Does_Nothing(double? lat, double? lon)
    {
        ...
        // No resolvable location (tenant default incomplete, no Farms/Units/Parcels) -> ResolveDistinctAsync returns empty, so WeatherLocationStateGetAsync/GetFrostForecastAsync would throw if called.
    }
    ...
    public async Task Cold_Clear_Calm_Forecast_Predicts_Frost_And_Alerts_Once()
    {
        ...
        _weatherClient.Setup(c => c.GetFrostForecastAsync(Lat, Lon, "test-key", 12, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new FrostForecastResult(1.0, 10.0, 0.5, 6));
        _tenants.Setup(t => t.WeatherLocationStateSetFrostAsync(TenantId, Lat, Lon, true, 6, It.IsAny<DateTimeOffset>())).Returns(Task.CompletedTask);
        ...
        _tenants.Verify(t => t.WeatherLocationStateSetFrostAsync(TenantId, Lat, Lon, true, 6, It.IsAny<DateTimeOffset>()), Times.Once);
    ...
    public async Task Any_Factor_Outside_Threshold_Does_Not_Predict_Frost(double tempC, double cloudPercent, double windMps)
    {
        ...
        _tenants.Setup(t => t.WeatherLocationStateSetFrostAsync(TenantId, Lat, Lon, false, null, It.IsAny<DateTimeOffset>())).Returns(Task.CompletedTask);

        await NewEvaluator().RunOnceAsync();

        _tenants.Verify(t => t.WeatherLocationStateSetFrostAsync(TenantId, Lat, Lon, false, null, It.IsAny<DateTimeOffset>()), Times.Once);
    ...
    public async Task Failed_Fetch_Leaves_Last_Known_State_Untouched()
    {
        ...
        // Strict mock: WeatherLocationStateSetFrostAsync would throw if called - a failed fetch must not overwrite the last good reading.
    }

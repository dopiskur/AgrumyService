    ...
    public class DeviceController(IApi api) : Controller
    {
        ...
        public async Task<ActionResult> Details(int? idDevice)
        {
            ...
            if (User.IsInRole(RoleNames.GlobalAdmin) || User.IsInRole(RoleNames.GlobalReader))
            {
                ...
                ViewBag.MqttCredentialsPendingSync = serverConfig.MqttTransportEnabled
                    && (serverConfig.MqttCredentialsSyncedAtUtc is not DateTimeOffset syncedAt || device.DateCreated > syncedAt);
            }
        ...
        public async Task<ActionResult> Simulation(int? idDevice, int? idSimulationSession)
        {
            ...
            // The "this can move real hardware" warning only makes sense for a physical device; a virtual one has no relay to move.

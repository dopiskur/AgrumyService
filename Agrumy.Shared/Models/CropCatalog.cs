namespace Agrumy.Shared.Models
{
    ...
    /// One BBCH stage's environmental parameters for one CropCatalogEntry (Type==Arable) - DurationDaysMin/Max is this stage's OWN length, not cumulative from Sowing.StartDate; summing every stage's duration (see ArableSeasons.cshtml's cropMaturityDays) estimates the crop's total days-to-maturity, and a running sum from StartDate lets the app estimate which stage a sowing currently sits in.
